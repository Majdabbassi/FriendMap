...
**Why Postgres vs Redis:** Postgres holds permanent truth — users, friendships, sharing settings, list entries. Redis holds only hot, ephemeral, constantly-changing state — each user's current position (with a 24h TTL, matching the spec's retention requirement directly) — because writing every 5-15-second GPS tick to a relational database at scale is unnecessary load for data that's replaced within seconds anyway.

## Data model

- **User** — email, username, password hash.
- **Friendship** — `requesterId`, `addresseeId`, `status` (PENDING/ACCEPTED/REJECTED). A friendship is only usable for sharing once ACCEPTED. Rejecting deletes the row entirely (as if the request never happened) rather than keeping a REJECTED record, so a new request afterward isn't blocked by the unique constraint on the requester/addressee pair.
...
- The client throttles its own emit rate to stay under the server's 4-second-per-update rate limit.
...
Second bottleneck: Postgres connection pooling under high friend-request/sharing-settings write volume — mitigated by the fact that the hottest path (location updates) never touches Postgres at all, only Redis; Postgres only sees the comparatively rare writes (friend actions, settings changes).
...
- **Location history table not implemented** — explicitly marked "nice to have" in the spec. The mandatory 24h-retention requirement is satisfied by Redis's TTL on current-location alone; adding durable history would need its own scheduled purge job (Postgres doesn't auto-expire rows), deferred as genuinely optional.
