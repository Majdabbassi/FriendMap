...
export class LocationHistoryService implements OnModuleInit, OnModuleDestroy {
  ...
  async enqueueIfSampled(userId: string, point: AcceptedPoint) {
    ...
    try {
      await this.locationRepository.createHistoryPoint(
        userId,
        {
          lat: point.lat,
          lng: point.lng,
        ...
        new Date(Date.now() + HISTORY_RETENTION_MS),
      );
    } catch (error) {
  ...
  async getHistory(userId: string, from?: Date, to?: Date) {
    ...
    const points = await this.locationRepository.findHistoryByUserId(
      userId,
      new Date(start),
      new Date(end),
    );
  ...
  private async cleanupExpired() {
    try {
      await this.locationRepository.deleteExpiredHistory();
    } catch (error) {
