...
function updateMarker(point: Point) {
  ...
  else markers.set(point.userId, L.marker([point.lat, point.lng], { icon: markerFor(point) }).addTo(map))
}
...
function startLocationWatch() {
  ...
  watchId = navigator.geolocation.watchPosition((position) => socket?.emit('location:update', { lat: position.coords.latitude, lng: position.coords.longitude, accuracy: position.coords.accuracy, timestamp: Date.now() }), () => showNotice('Location permission is needed to share your position.'), { enableHighAccuracy: true, maximumAge: 10_000 })
}
onMounted(async () => {
  try {
    friends.value = await apiRequest<Friendship[]>('/friendships')
    if (mapElement.value) map = L.map(mapElement.value).setView([20, 0], 2)
    ...
    socket.on('location:hidden', ({ userId }: { userId: string }) => { markers.get(userId)?.remove(); markers.delete(userId); points.delete(userId); showNotice(`${usernameFor(userId)} is no longer sharing their location.`) })
...
<template>
  <section class="map-page"><div class="map-toolbar"><div><p class="eyebrow">LIVE VIEW</p><h1>Friend map</h1></div><span class="status"><i :class="connection"></i>{{ connection }}</span></div><div ref="mapElement" class="map-canvas"></div><div v-if="notice" class="toast">{{ notice }}</div><div class="map-legend"><span class="legend-dot"></span> Shared location <span class="legend-dot stale-dot"></span> Stale for 60m+</div></section>
</template>